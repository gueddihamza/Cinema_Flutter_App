...
class _roomPageState extends State<roomPage> {
                  ...
                  return Card(
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        if (this.roomList[index]['movieProjections'] != null)
                          Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: Row(
                              ...
                              children: <Widget>[
                                Image.network(
                                  GlobalData.host +
                                      "/movieImage/${this.roomList[index]['currentProjection']['movie']['id']}",
                                  width: 150,
                                  height: 250,
                                ),
                                Column(
                                  children: <Widget>[
                                    ...(this.roomList[index]['movieProjections']
                                            as List<dynamic>)
                                        .map((projection) {
                                      return RaisedButton(
                                        color: (this.roomList[index]
                                            ...
                                            : Colors.purple[200],
                                        child: Text(
                                          "${projection['session']['startTime']}"
                                              .substring(0, "${projection['session']['startTime']}".length - 3) +
                        ...
                        if (this.roomList[index]['currentProjection'] != null &&
                          ...
                          Column(
                            ...
                            /*  Container(
                              ...
                              Wrap(
                                children: <Widget>[
                                  ...this.roomList[index]['currentProjection']['listTickets'].map((ticket) {
  ...
  void loadRooms() {
    String url = this.widget.cinema['_links']['rooms']['href'];
    http.get(url).then((resp) {
      setState(() {
        String body = utf8.decode(resp.bodyBytes);
        this.roomList = json.decode(body)['_embedded']['rooms'];
      });
    }).catchError((err) {
      print(err);
    });
  ...
  void loadProjections(room) {
    String url = room['_links']['movieProjections']['href']
        .toString()
        .replaceAll("{?projection}", "?projection=p1");
    http.get(url).then((resp) {
      setState(() {
        room['movieProjections'] =
            json.decode(resp.body)['_embedded']['movieProjections'];
        room['currentProjection'] = room['movieProjections'][0];
        room['currentProjection']['listTickets'] = [];
      });
    }).catchError((err) {
      print(err);
    });
  ...
  void loadTickets(projection, room) {
    ...
    http.get(url).then((resp) {
      setState(() {
        projection['listTickets'] = json.decode(resp.body)['_embedded']['tickets'].where((ticket)=>ticket["booked"] == false);
  ...
  int countAvailableSeats(projection) {
    ...
    for (int i = 0; i < projection['tickets'].length; i++) {
      if (projection['tickets'][i]['booked'] == false)
        count++;
    }
    return count;
  }
  ...
  void buyTickets(clientName , paymentCode , tickets , index) {
      Map data={"clientName":clientName ,"paymentCode":paymentCode, "tickets" : tickets};
      String body=json.encode(data);
      http.post(GlobalData.host+"/payTickets",headers: {"Content-Type": "application/json"} , body: body)
